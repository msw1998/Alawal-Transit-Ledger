...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (event.httpMethod === 'POST') {
      ...
      if (!name?.trim()) return { statusCode: 400, headers: HEADERS, body: JSON.stringify({ error: 'Name is required' }) };
      const { data, error } = await supabase
        .from('materials')
        .insert({ name: name.trim(), org_id: ctx.orgId })
        .select('id, name')
        .single();
      if (error && error.code === '23505') return { statusCode: 409, headers: HEADERS, body: JSON.stringify({ error: 'Material already exists' }) };
      if (error) throw error;
      return { statusCode: 200, headers: HEADERS, body: JSON.stringify(data) };
    }
    ...
    if (event.httpMethod === 'DELETE') {
      ...
      const { error } = await supabase.from('materials').delete().eq('id', id).eq('org_id', ctx.orgId);
      if (error) throw error;
    ...
    console.error('materials fn error:', err);
